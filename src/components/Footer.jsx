		...
		const urlItem = urlList.map((urlObj, index) => {
			return (
				<li key={index} className="nav-item mb-2">
					<a className="color-1 text-type-1 cstm-link" href={urlObj.url}>
						{urlObj.name}
					</a>
				</li>
			);
		...
		const item = footerUrls.map((footerUrl, index) => {
			return (
				<div className="col-4" key={index}>
					<h4 className="fw-meduim text-type-3 color-2 mb-3">
						{footerUrl.title}
					</h4>
					<nav>
						<ul className="navbar-nav">{urlRender(footerUrl.urls)}</ul>
					</nav>
				</div>
			);
		});
		...
		<footer className="bg-color-1 cstm-mt-1">
				...
				<div className="p-4 p-md-5 bg-white rounded-3 position-absolute top-0 start-50 translate-middle w-100 sub-card">
					<div className="row align-items-center g-0">
						<div className="col-sm-7 col-md-8">
							<h2 className="hd-type-2 fw-medium mb-2 mb-md-3 color-2">
								Subscribe Now for <br /> Get Special Features!
							</h2>
							<p className="text-type-1 mb-0 fw-regular color-1">
			...
			<div className="container">
				<div className="row flex-column-reverse flex-md-row g-4">
					<div className="col-md-6">
						<a className="navbar-brand" href="#">
							<span className="d-flex align-items-center gap-2">
								...
								<p className="mb-0 text-type-3 fw-meduim">
									Lasles<strong>VPN</strong>
								</p>
							</span>
						</a>
						<p className="my-3 text-type-1 fw-regular color-1">
							{" "}
					...
					<div className="col-md-6">
						<div className="row">{footerUrlRender()}</div>
					</div>
				</div>
			</div>
		</footer>
