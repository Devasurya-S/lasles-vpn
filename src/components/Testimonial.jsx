  ...
  return (
    <section className='bg-white'>
        <div className='container-md py-3 py-lg-5'>
            <h2 className='hd-type-2 fw-medium mb-2 mb-md-3 color-2 text-center'>Trusted by Thousands of <br/> Happy Customer</h2>
            <p className='text-type-1 fw-regular mb-3 mb-md-4 color-1 text-center mb-4 mb-md-5'>These are the stories of our customers who have joined us with great<br/> pleasure when using this crazy feature.</p>
            <Slider {...settings}>
                {data.map((item, index) => {
                  return (
                    <div key={index}  className='p-3 h-100'>   
                      <div className='d-flex gap-3'>
                        <div>
                          <img src={item.imgUrl} className='img-fluid' alt='profile icon'/>
                        </div>
                        <div className='d-flex gap-3 flex-grow-1 justify-content-between align-items-center'>
                          <div>
                            <h4 className='mb-0 text-type-3 color-2 fw-medium'>{item.name}</h4>
                            <p className='mb-0 text-type-4 fw-regular'>{item.place}</p>
                          </div>
                      ...
                      <p className='mb-0 mt-2 text-type-1 fw-regular color-1'>{item.content}</p>
                    </div>
                  )
                })}
            </Slider>
